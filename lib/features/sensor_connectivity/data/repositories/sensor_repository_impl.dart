...
class SensorRepositoryImpl implements SensorRepository {
  ...
  Stream<List<SensorDevice>> scanForDevices() {
    // TODO: Implement BLE scanning logic if still needed alongside Firestore
  ...
  Future<Either<Failure, bool>> connectToDevice(String deviceId) async {
    try {
      ...
      return Left(ConnectionFailure('Failed to connect via BLE: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> disconnectFromDevice(String deviceId) async {
    try {
      ...
      return Left(ConnectionFailure('Failed to disconnect via BLE: ${e.toString()}'));
    }
  }

  @override
  Stream<SensorData> subscribeToSensorData(String deviceId) {
    return bleDatasource.subscribeToSensorData(deviceId).map((data) => 
      SensorData(
        deviceId: deviceId,
        data: {'rawData': data},
  ...
  @override
  Future<Either<Failure, void>> sendCommandToDevice(String deviceId, String command) async {
    try {
      await bleDatasource.sendCommandToDevice(deviceId, command);
      return const Right(null);
    } catch (e) {
      return Left(CommunicationFailure('Failed to send command via BLE: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, List<String>>> discoverWifiNetworks(String deviceId) async {
    try {
      final networks = await bleDatasource.discoverWifiNetworks(deviceId);
      return Right(networks);
    } catch (e) {
      return Left(CommunicationFailure('Failed to discover Wi-Fi networks: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, bool>> pairWifiNetwork(String deviceId, String ssid, String password) async {
    try {
      final result = await bleDatasource.pairWifiNetwork(deviceId, ssid, password);
      return Right(result);
    } catch (e) {
      return Left(CommunicationFailure('Failed to pair Wi-Fi network: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> setBleNotification(
      BluetoothDevice device, Guid serviceUuid, Guid characteristicUuid, bool enable) async {
    try {
      await bleDatasource.setBleNotification(device, serviceUuid, characteristicUuid, enable);
      return const Right(null);
    } catch (e) {
      return Left(CommunicationFailure('Failed to set BLE notification: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> writeBleCharacteristic(
    ...
    try {
      await bleDatasource.writeBleCharacteristic(
          device, serviceUuid, characteristicUuid, value, withoutResponse: withoutResponse);
      return const Right(null);
    } catch (e) {
      return Left(CommunicationFailure('Failed to write to BLE characteristic: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> requestBlePermissions() async {
    try {
      ...
      return Left(PermissionFailure('Failed to request BLE permissions: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> startBleScan() async {
    try {
      ...
      return Left(CommunicationFailure('Failed to start BLE scan: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> stopBleScan() async {
    try {
      ...
      return Left(CommunicationFailure('Failed to stop BLE scan: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> connectBleDevice(BluetoothDevice device) async {
    try {
      final result = await bleDatasource.connectToDevice(device.id.toString());
      return result ? const Right(null) : Left(ConnectionFailure('Failed to connect to BLE device'));
    } catch (e) {
      return Left(ConnectionFailure('Failed to connect to BLE device: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, void>> disconnectBleDevice(BluetoothDevice device) async {
    try {
      await bleDatasource.disconnectFromDevice(device.id.toString());
      return const Right(null);
    } catch (e) {
      return Left(ConnectionFailure('Failed to disconnect BLE device: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, bool>> connectToMqttBroker(
      String broker, int port, String clientId, String? username, String? password) async {
    try {
      final result = await mqttDatasource.connect(broker, port, clientId, username, password);
      return Right(result);
    } catch (e) {
      return Left(ConnectionFailure('Failed to connect to MQTT: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, void>> disconnectFromMqttBroker() async {
    try {
      ...
      return Left(ConnectionFailure('Failed to disconnect from MQTT: ${e.toString()}'));
    }
  }
  ...
  @override
  Future<Either<Failure, void>> publishToMqttTopic(String topic, String message) async {
    try {
      mqttDatasource.publish(topic, message);
      return const Right(null);
    } catch (e) {
      return Left(CommunicationFailure('Failed to publish to MQTT: ${e.toString()}'));
    }
  }
  ...
  Stream<List<SensorDevice>> streamFirestoreDevices() {
    try {
      ...
      return Stream.error(DatabaseFailure('Failed to stream devices: ${e.toString()}'));
    }
  }
  ...
  Stream<List<DeviceLog>> streamFirestoreDeviceLogs(String deviceId) {
     try {
      ...
      return Stream.error(DatabaseFailure('Failed to stream logs for device $deviceId: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, void>> addDeviceToFirestore(SensorDevice device) async {
    try {
  ...
  @override
  Future<Either<Failure, void>> updateDeviceStatusInFirestore(String deviceId, Map<String, dynamic> statusData) async {
     try {
      await firestoreDatasource.updateDeviceStatus(deviceId, statusData);
      return const Right(null);
    } catch (e) {
      return Left(DatabaseFailure('Failed to update device status: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, void>> addDeviceLogToFirestore(String deviceId, DeviceLog log) async {
     try {
  ...
  @override
  Future<Either<Failure, void>> updateDeviceSettingsInFirestore(String deviceId, Map<String, dynamic> settings) async {
    try {
      await firestoreDatasource.updateDeviceSettings(deviceId, settings);
      return const Right(null);
    } catch (e) {
      return Left(DatabaseFailure('Failed to update device settings: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, Map<String, dynamic>?>> getDeviceSettingsFromFirestore(String deviceId) async {
    try {
